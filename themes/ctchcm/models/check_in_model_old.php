...
//        $linkDelete = wp_nonce_url($linkDelete, $action, $name);
    ...
    public function column_checkin($item) {
        ...
        $paged = max(1, @$_REQUEST['paged']);
        $name = 'security_code';
        $linkStatus = add_query_arg(array('action' => 'uncheckin', 'id' => $item['ID'], 'paged' => $paged));
...
// XOA HINH DAI DIEN
        ...
        // XOA HINH BARCODE
        if (is_file(HCM_DIR_IMAGES . 'barcode/' . $barcode)) {
            unlink(HCM_DIR_IMAGES . 'barcode/' . $barcode);
        }
    }
    ...
    public function saveItem($arrData = array(), $option = array()) {
        ...
        if (empty($arrData['hidden_barcode'])) {
            ...
            file_put_contents(HCM_DIR_IMAGES . 'barcode/' . $bar . '.png', $generatorPNG->getBarcode($bar, $generatorPNG::TYPE_CODE_128, 2, 30));
            $barcode = $bar;
        } else {
